...
def handle_smokeapi_operation(
    ui, helper, install_path, steam_api_files, game_name, is_install=True
):
    ...
    try:
        with ui.create_status_context(
            f"{'Installing' if is_install else 'Uninstalling'} SmokeAPI..."
        ):
            if is_install:
        ...
        if success:
            ui.show_success(
                f"Successfully {'installed' if is_install else 'uninstalled'} SmokeAPI!"
            )
        else:
            ui.show_error(
                f"Failed to {'install' if is_install else 'uninstall'} SmokeAPI"
            )
...
def main():
    ...
    parser.add_argument(
        "--manual",
        metavar="steamapps_path",
        help="Sets the steamapps path for faster operation",
        required=False,
    )
    ...
    try:
        ...
        if not library_folders:
            if args.manual:
                ui.show_error(
                    f"Could not find Steam library at specified path: {args.manual}"
                )
            else:
                ui.show_warning(
                    "No Steam library folders found. Please enter the path manually."
                )
                    ...
                    if is_installed:
                        ...
                        if action == "1":  # Fetch DLCs
                            ...
                            if ui.get_user_confirmation(
                                ...
                                with ui.create_status_context(
                                    "Uninstalling CreamLinux..."
                                ):
                                    success = helper.uninstall_creamlinux(
                                        game_info[1][2]
                                    )
                                if success:
                                    ui.show_success(
                                        f"Successfully uninstalled CreamLinux from {game_info[1][0]}"
                                    )
                        ...
                        handle_dlc_operation(
                            ui, helper, game_info[0], game_info[1][0], game_info[1][2]
                        )
        ...
        if isinstance(e, (RequirementsError, NetworkError, InstallationError)):
            ...
            ui.show_error(
                f"An unexpected error occurred: {str(e)}", show_exception=args.debug
            )
